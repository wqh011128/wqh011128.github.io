...
计算方式在[Minimax-01](https://alidocs.dingtalk.com/document/edit?docKey=mxPOG5zNPK7a2nKa&dentryKey=4mN9O29rF0ZbYNZJ&type=d)梳理中解释过，可以分为左乘和右乘两个大类。
...
- **有损压缩：****Lightning Attention, Linear Attention...(delete Softmax !) - 右乘法**
  - 维护一个固定大小的矩阵状态S（和K^TV保持相同的维度）；
  - 每次进来一个新的token，就用S_{ new} =S_{old} + K^TV来更新；
  - 计算attention时直接用Q乘以S；
...
关键点：给旧状态S_{t-1}添加decay，这里的decay用标量\alpha_{t}来表示。
...
Author's explain of deltanet：https://sustcsonglin.github.io/blog/2024/deltanet-1/
...
令状态矩阵 S = \sum_{i} v_ik^T_i,并且所有的向量都是单位向量，

这时，如果信息没有损失，从S中查询k_j应当得到v_j,但实际上并非如此，如下图

（来源于https://sustcsonglin.github.io/blog/2024/deltanet-1/）：
...
为了消除error，需要让k^T_ik_j=0,i≠j。也就是说，token i 和token j 必须是正交的。
...
- S_{t-1}k_t相当于k_t检索出的“old value”，积累着过去的记忆和检索error。
- v_t表示理想的检索值。
...
定义：c_t =[M_t, X_t]，因此，S_t可以通过c_t的确定而确定。

将迭代公式看作一步更新对应的函数：f_t(S_{t-1}) = S_{t-1} M_t + X_t

假设初始状态为S，继续定义区间 [0:1] 的“复合变换”f_{0:1}(S) = f_1\!\bigl(f_0(S)\bigr)

将总变换写回 SM+X 的形式f_{0:1}(S) = S M_{0:1} + X_{0:1}，如下：

\begin{aligned} f_1\!\bigl(f_0(S)\bigr) &= f_1(SM_0 + X_0) \\ &= (SM_0 + X_0)M_1 + X_1 \\ &= S(M_0M_1) + (X_0M_1 + X_1). \end{aligned}

因而得到区间 [0:1] 的合成对c_{[0:1]} = [\,M_0M_1,\; X_0M_1 + X_1\,]
...
设q_t, k_t, v_t \in \mathbb{R}^{d\times 1}（列向量）,
...
Q_{[i]} = [q_{[i]}^1,\dots,q_{[i]}^C] \in \mathbb{R}^{d\times C}\newline K_{[i]} = [k_{[i]}^1,\dots,k_{[i]}^C] \in \mathbb{R}^{d\times C}\newline V_{[i]} = [v_{[i]}^1,\dots,v_{[i]}^C] \in \mathbb{R}^{d\times C}\newline O_{[i]} = [o_{[i]}^1,\dots,o_{[i]}^C] \in \mathbb{R}^{d\times C}.
...
S_{[i]}^r := S_{[i]} + \sum_{t=1}^{r}v_{[i]}^t(k_{[i]}^t)^T\in\mathbb{R}^{d\times 1}.
...
o_{[i]}^r =S_{[i]}^rq_{[i]}^r= S_{[i]}q_{[i]}^r + \sum_{t=1}^{r}v_{[i]}^t(k_{[i]}^t)^Tq_{[i]}^r.
...
O_{[i]}=S_{[i]}Q_{[i]}+\sum_{r=1}^{C}\sum_{t=1}^{r}v_{[i]}^t(k_{[i]}^t)^Tq_{[i]}^r
...
\tilde Q_{[i]} := Q_{[i]}^\top\in\mathbb{R}^{C\times d},\quad \tilde K_{[i]} := K_{[i]}^\top\in\mathbb{R}^{C\times d},\quad \tilde V_{[i]} := V_{[i]}^\top\in\mathbb{R}^{C\times d},\quad \tilde O_{[i]} := O_{[i]}^\top\in\mathbb{R}^{C\times d}.
...
给Delta Network添加了门控单元\alpha_t控制状态衰减。
...
关键：将Gated Delta Network (GDN) 中的标量门控单元替换成细粒度对角化门控Diag ( αt )。

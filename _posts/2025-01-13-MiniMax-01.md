...
对于token来说，分配到第i个expert的token占比为：
...
但是在有因果掩码的情况下，Linear attention阻碍了并行实现。具体来说，
目标：计算右乘K^TV 。\newline
在推理阶段， 假设M_i = \sum_{t=1}^{i}K^T_tV_t，这里K的维度是从（1,D）到（i,D）。
\newline
...
详细推导请查看回顾Linear Attention的并行化计算-Chunkwise Parallelhttps://alidocs.dingtalk.com/document/edit?docKey=KM7qeobwYDKYplpj&dentryKey=k9zgV53ocPvE7Yv4&type=d&anchorId=uu_mm4okfsl3g8o0pazvef
...
GPU 先处理那 18 个 Decoding 的请求，再处理那 2 个 Prefill 的请求。
    a. 处理 Prefill 时：计算核心跑满。
    b. 处理 Decoding 时：显存带宽跑满，但大部分计算核心（SMs）是闲置的。

...
其中 `Linear-1` 和 `Linear-2` 是 expert FFN 内部天然存在的两个 GEMM，不是额外引入的新结构。如果 expert 使用 SwiGLU，那么 `Linear-1` 通常同时产生 gate/up 两路中间表示，`SwiGLU + FP8 Cast` 把中间激活重新量化后交给 `Linear-2`。

### 9.1 wave 到底是什么

DeepSeek-V4 的关键做法是把 local experts 进一步切成多个 **expert waves**。一个 wave 不是 token chunk，也不是某个 expert 的一个 GEMM tile，而是一小批 experts 及其对应的 routed tokens。

论文里的调度逻辑可以理解成：

```text
wave 0 的 experts 完成 dispatch -> 立刻开始 Linear-1 / Act / Linear-2
...
```text
combine previous wave | compute current wave | dispatch next wave
```

这就是它和普通 EP overlap 的区别。普通做法经常是“整层 dispatch 完成后再算，整层算完后再 combine”；DeepSeek-V4 让每个 wave 准备好就进入计算，不等其他 experts。这样可以减少 long-tail expert 或小 batch 场景下的空转，论文特别提到 RL rollout 和 high-speed agent serving 这类 latency-sensitive 场景会受益。

### 9.2 为什么通信能被盖住

论文给出的判断不是“带宽越高越好”，而是看计算通信比：

$$
\frac{C}{B}\leq \frac{V_{\mathrm{comp}}}{V_{\mathrm{comm}}}
$$

其中 $C$ 是设备峰值计算能力，$B$ 是互联带宽，$V_{\mathrm{comp}}$ 是 MoE 计算量，$V_{\mathrm{comm}}$ 是 MoE 通信量。

对 DeepSeek-V4-Pro，一个 token-expert pair 的 SwiGLU expert 大约需要：

$$
V_{\mathrm{comp}}\approx 6hd
$$

这里的 $h$ 是 hidden size，$d$ 是 intermediate hidden size。三项来自 gate projection、up projection、down projection。通信量约为：

$$
V_{\mathrm{comm}}\approx 3h
$$

这里对应 FP8 dispatch 和 BF16 combine。因此：
...
含义是：如果硬件的 $C/B$ 不超过这个比例，那么通信理论上可以被 expert GEMM 覆盖；继续堆更多互联带宽的收益会下降，反而要保证计算、显存和网络能同时高负载运行。

### 9.3 公开代码能看到什么

论文说已经开源 CUDA-based mega-kernel **MegaMoE2**，位置在 DeepGEMM。DeepGEMM PR #304 对这个 kernel 的描述很直接：它把 `dispatch / linear 1 / SwiGLU / linear 2 / combine` 融合进一个 mega-kernel，并重叠 NVLink communication 和 tensor core computation。

公开测试文件 `tests/test_mega_moe.py` 能看到两个路径的对照：
...
# legacy baseline
recv_x, handle = ep_buffer.dispatch(...)
l1_y = grouped_fp8_fp4_gemm(recv_x, l1_weights, ...)
l1_y = swiglu_and_cast(l1_y, topk_weights, ...)
l2_y = grouped_fp8_fp4_gemm(l1_y, l2_weights, ...)
y = ep_buffer.combine(l2_y, handle=handle)
```

这段对照很有用：它说明 MegaMoE2 不是单独优化某一个 GEMM，而是把 EP dispatch、两次 expert GEMM、SwiGLU/cast、combine 都放进同一个 fused runtime 里处理。Python API 层没有暴露 `num_waves` 这种模型超参数；wave 更像 kernel 内部的调度粒度，由 routed token metadata、local expert receive stats、symmetric buffer 和 transformed expert weights 共同驱动。

PR #316 的 benchmark 也和论文说法一致：在 EP8 下，DeepSeek-V4-Flash 使用 256 experts、top-k=6、hidden size 4096、intermediate size 2048；DeepSeek-V4-Pro 使用 384 experts、top-k=6、hidden size 7168、intermediate size 3072。不同 batch size 下相对 legacy baseline 大约有 1.50x 到 1.96x 的加速，其中小 batch latency 场景收益更明显。

### 9.4 论文给硬件和 kernel 的几个观察

DeepSeek-V4 这节最后还有几个系统侧观察，值得单独记下来：

- **不是无限堆带宽。** 当 $C/B$ 已经低于 $2d$ 这个阈值后，继续增加互联带宽的边际收益下降；更重要的是让 compute、memory、network 同时保持高负载。
- **power budget 会变成瓶颈。** 极端融合会让 Tensor Core、HBM、NVLink 同时繁忙，功耗墙可能比单看 FLOPs 或带宽更早出现。
- **dispatch 采用 pull-based 思路。** 论文说 dispatch 阶段由每个 GPU 主动读取远端 activations，避免 fine-grained push 带来的高通知延迟；如果未来硬件有更低延迟的跨 GPU signaling，push 才会更自然。
- **SwiGLU 本身也可能阻塞 pipeline。** 因为 SwiGLU 有指数和除法相关开销，论文建议未来可以考虑更低成本的 element-wise activation，减少 post-GEMM 处理对 GEMM pipeline 的打断。
...
| MoE Runtime Optimization | expert parallelism 下通信阻塞计算 | expert waves + MegaMoE2 fused runtime | kernel 调度和实现复杂度更高 | DeepGEMM `fp8_fp4_mega_moe` |
...
- DeepGEMM MegaMoE2 PR: [deepseek-ai/DeepGEMM#304](https://github.com/deepseek-ai/DeepGEMM/pull/304)
- DeepGEMM MegaMoE2 Benchmark PR: [deepseek-ai/DeepGEMM#316](https://github.com/deepseek-ai/DeepGEMM/pull/316)
